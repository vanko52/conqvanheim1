...
export default function StatRunes() {
    ...
    // subscribe to global DnD events only while this page is mounted
    useEffect(() => {
            ...
            <div className="box">
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                    ...
                    <Droppable droppableId="list">
                        {prov => (
                            <div ref={prov.innerRef} {...prov.droppableProps} style={{ width: 180, maxHeight: 300, overflowY: 'auto', border: '1px solid #000', padding: 6 }}>
                                <strong>Stash</strong>
                                {stash.map((it, idx) => (
                                    <Draggable key={it.id} draggableId={String(it.id)} index={idx}>
                                        {p => (
                                            <div ref={p.innerRef} {...p.draggableProps} {...p.dragHandleProps} className={`rune-pill ${it.rune.rarity}`}>
                    ...
                    <Droppable droppableId="slot">
                        {prov => (
                            <div ref={prov.innerRef} {...prov.droppableProps} className={`slot-square ${stat} ${shake ? 'shake' : ''}`} style={{ width: 180, height: 180 }}>
                                <strong>Equipped</strong>
                                {equipped && (
                                    <Draggable draggableId={String(equipped.inventoryId)} index={0}>
                                        {p => (
                                            <div ref={p.innerRef} {...p.draggableProps} {...p.dragHandleProps} className={`rune-pill ${equipped.inventory.rune.rarity} equipped`}>
...
// Add to global CSS (index.html <style>)
// -------------------------------------------------
// @keyframes shake {
...
// }
// .slot-square.shake { animation: shake 0.4s ease; }
