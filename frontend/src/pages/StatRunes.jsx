// StatRunes.jsx – two boxes with guard + shake
    ...
    async function onDragEnd(res){
        ...
        try{
            if (src==='list' && dst==='slot') {
                if (equipped) {                         // slot already filled
    ...
    return (
        <ErrorBoundary fallback={<p style={{color:'red'}}>Drag-drop crashed — refresh</p>}>
            <div className="box">
                ...
                {/* key={stat} forces full remount → prevents blank screen */}
                <DragDropContext key={stat} onDragEnd={onDragEnd}>
                        ...
                        <Droppable droppableId="list">
                            {prov=>(
                                <div ref={prov.innerRef}
                                     {...prov.droppableProps}
                                     style={{width:180,maxHeight:300,overflowY:'auto',
                                         border:'1px solid #000',padding:6}}>
                                    <strong>Stash</strong>
                                    {stash.map((it,idx)=>(
